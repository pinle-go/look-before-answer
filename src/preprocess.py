    ...
    with open(filename, "r") as fh:
        ...
        for article in tqdm(source["data"]):
            for para in article["paragraphs"]:
                ...
                # we store spans because we char position of answer start and the answer in the data
...
def preprocess(args, config):
    ...
    dev_meta = build_features(
        config, dev_examples, "dev", config.dev_file, word2idx_dict, char2idx_dict, is_test=True
    )
    if os.path.exists(config.raw_test_file):
